...
export const AssignTask = ({
  ...
  useEffect(() => {
    ...
    console.log('currentState123123: ', currentState);
    // if (!isAssigningTask && currentState?.subMatrics.assignTaskStatus === 'ASSIGN_TASK_REQUEST') {
    //   Alert.alert('Task successfully assigned.');
  ...
  const RenderNoDataMsg = () => {
    return (
      <Layout style={styles.noDataMsgWrap}>
        <Text category="h4" status="info" style={{ color: 'grey' }}>
          No data available!
        </Text>
      </Layout>
    );
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: '#fff' }}>
      ...
      <Layout style={[styles.taskDataWrap]}>
        <Layout style={[styles.tasks]} level="1">
          ...
          <Collapse onToggle={() => { setVisible(!visible); }}
            style={styles.collaps}>
            <CollapseHeader style={styles.header}>
